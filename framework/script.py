...
capabilities = []
# after waiting this many seconds without working on a job, the worker will exit; if -1, the worker never exits
maxIdleTime = 3600 
maxSimTime = 60*10
clearTmpDir = False
numberOfWorker = 1
...
token = ''
headers = {'Content-Type':'application/json', 'Worker-Token': '?'}
...
def configure():
	...
	global serverUri, capabilities, maxIdleTime, maxSimTime, executedInstallScripts, numberOfWorker
...
def registerSelfAsWorker():
	global token
	server = httplib.HTTPConnection(serverUri)
	registration_payload = json.dumps({'Capabilities': capabilities})
	server.request('POST', '/workers', registration_payload, headers)
	response = server.getresponse()
	if response.status == 201:
		data = json.loads(response.read())
		token = data['Token']
		headers['Worker-Token'] = token
		print 'Registered at server, token: %s' % token
	else:
		print 'Could not register as worker!'
		pprint(response.read())

def findAndExecutePendingJob():
        global token
	server = httplib.HTTPConnection(serverUri)
	server.request('GET', '/random_job', headers=headers)
	response = server.getresponse()
	location = response.getheader('Location')
	if response.status == 401 or response.status == 403:
		raise WorkerNotRegisteredException
	elif response.status == 302 and location != None:
		print 'Pending job found at', location, 'for', token
		executeJob(location)
		return True
	else:
		print 'No pending simulation at this time...'
		return False
...
def executeJob(job_location):
	# download zip file with the simulation
	server = httplib.HTTPConnection(serverUri)
	server.request('GET', job_location + '/simulation.zip')
	response = server.getresponse()
	...
	if result == 0:
		print 'Executing...'
		try:
			result = subprocess32.call('python simulation.py',
				shell=True,
				stdout=logfile,
				stderr=subprocess.STDOUT,
				timeout=maxSimTimeConfig,
	...
	if result == 0:
		...
		server.request('PUT', job_location + '/results', payload, headers)
		response = server.getresponse()
		if response.status == 401 or response.status == 403:
			raise WorkerNotRegisteredException
		...
		print 'An error occured in simulation %s' % job_location
		print log_content
		error_payload = json.dumps({'ErrorLog':log_content})
		server = httplib.HTTPConnection(serverUri)
		server.request('PUT', job_location + '/error', error_payload, headers)
		response = server.getresponse()
		if response.status == 401 or response.status == 403:
			raise WorkerNotRegisteredException
...
if __name__ == '__main__':
	...
	while True:
		...
		try:
			jobWasFound = findAndExecutePendingJob()
